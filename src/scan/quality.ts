...
export function aggregateCreators(
  ...
  const latest = latestByContract(backfills);
  const byOwner = new Map<string, { drops: CreatorFact[]; sales: number | null; net: number | null; mints: number }>();

  for (const fact of facts) {
    const owner = (fact.owner || "").trim().toLowerCase();
    if (!owner) continue;
    const group = byOwner.get(owner) ?? { drops: [], sales: null, net: null, mints: 0 };
    group.drops.push(fact);

    const record = latest.get(`${fact.chain}|${fact.contract.toLowerCase()}`);
    if (record) {
      if (record.salesCount !== null) group.sales = (group.sales ?? 0) + record.salesCount;
      if (record.netUsd !== null) group.net = (group.net ?? 0) + record.netUsd;
    }
    const entry = ledger ? entryOf(ledger, fact.chain, fact.contract) : undefined;
    if (entry?.status === "SUCCESS") group.mints += entry.quantity;
    byOwner.set(owner, group);
  }

  const stats = new Map<string, CreatorStats>();
  for (const [owner, group] of byOwner) {
    const withSupply = group.drops.filter((drop) => drop.maxSupply !== null && drop.maxSupply > 0n);
    const velocities = group.drops.map((drop) => drop.velocity24h).filter((v): v is bigint => v !== null);
    stats.set(owner, {
      owner,
      dropCount: group.drops.length,
      ...
      salesCount: group.sales,
      ownMints: group.mints,
      ownNetUsd: group.net,
      ownData: group.mints > 0 || group.net !== null || group.sales !== null,
    });
  }
  return stats;
}
...
export type Penalty = "stale" | "concentrated" | "no-socials";
...
function demandScore(input: QualitySignals): number | null {
  if (input.velocity24h === null || input.maxSupply === null || input.maxSupply <= 0n) return null;
