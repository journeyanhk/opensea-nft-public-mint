...
async function api(slug: string, suffix = "", body?: object): Promise<any> {
  ...
  if (!key) throw new Error("Cần OPENSEA_API_KEY trong .env để kiểm tra/mint Allowlist.");
  ...
  if (!response.ok) {
    const reasons: Record<number, string> = {
      401: "API key không hợp lệ", 403: "OpenSea từ chối truy cập", 404: "Không tìm thấy drop",
      409: "Drop chưa mở, đã kết thúc hoặc tạm dừng",
      422: "Không tạo được mint: có thể ví không thuộc allowlist, hết hạn mức/supply hoặc thiếu số dư",
      429: "OpenSea giới hạn tần suất, hãy thử lại sau",
    };
    throw new MintApiError(response.status, `${reasons[response.status] || "Lỗi OpenSea API"} (HTTP ${response.status}). Chưa xác nhận eligibility.`);
  }
  return response.json();
}

export function validateAllowlistTx(raw: any, contract: string, chain: string, wallet: string, quantity: number) {
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 100) throw new Error("Số lượng phải từ 1 đến 100.");
  if (raw.chain !== chain || getAddress(raw.to) !== getAddress(SEADROP_ADDRESS)) throw new Error("Giao dịch sai chain/SeaDrop.");
  if (typeof raw.value !== "string" || !/^\d+$/.test(raw.value)) throw new Error("Giá trị giao dịch không hợp lệ.");
  const decoded = allowlistInterface.parseTransaction({ data: raw.data });
  if (!decoded) throw new Error("Không phải giao dịch Allowlist được hỗ trợ.");
  const a = decoded.args;
  if (getAddress(a.nftContract) !== getAddress(contract) || a.quantity !== BigInt(quantity)) throw new Error("Sai collection/số lượng.");
  if (a.minterIfNotPayer !== ZeroAddress && getAddress(a.minterIfNotPayer) !== getAddress(wallet)) throw new Error("Sai ví nhận NFT.");
  const p = a.mintParams;
  const value = BigInt(raw.value);
  if (value !== p.mintPrice * BigInt(quantity) || p.feeBps > 10_000n || p.dropStageIndex === 0n) throw new Error("Thông số mint không hợp lệ.");
  const now = BigInt(Math.floor(Date.now() / 1000));
  if (now < p.startTime || now >= p.endTime) throw new Error("Vòng Allowlist chưa mở hoặc đã kết thúc.");
...
export async function checkAllowlist(slug: string, address: string, quantity: number, allowPublic = false) {
  ...
  if (!chain || typeof drop.contract_address !== "string") throw new Error("Drop/chain chưa được hỗ trợ.");
  ...
  if (isPublic && !allowPublic) throw new Error("API chọn vòng Public; chế độ này chỉ kiểm tra Allowlist.");
  ...
  for (const url of urls) {
    ...
    try {
      ...
      if (network.chainId !== BigInt(chain.chainId)) throw new Error("RPC sai chain.");
      if (isPublic) {
        ...
        if (!plan || raw.chain !== chain.key || getAddress(raw.to) !== getAddress(plan.to) ||
          ...
          throw new Error("Dữ liệu Public API không khớp on-chain.");
        }
        tx = { to: plan.to, data: plan.data, value: plan.value, method: "mintPublic", stage: "0" };
      }
      if (!tx) throw new Error("Thiếu dữ liệu mint.");
      await provider.call({ from: wallet, to: tx.to, data: tx.data, value: tx.value });
      console.log(`✓ Eligible hiện tại: ${wallet} | ${tx.method} | stage ${tx.stage} | ${quantity} NFT | ${formatEther(tx.value)} ${chain.nativeSymbol}`);
  ...
  throw new Error("OpenSea trả về dữ liệu mint nhưng mô phỏng on-chain chưa thành công. Chưa xác nhận mint được; không gửi giao dịch.");
}

export function hasLivePresale(drop: any, now = Date.now()): boolean {
  if (!Array.isArray(drop.stages)) throw new Error("OpenSea thiếu lịch mint; không thể tự chọn vòng.");
  return drop.stages.some((stage: any) => {
    const start = Date.parse(stage.start_time);
    const end = Date.parse(stage.end_time);
    if (!Number.isFinite(start) || !Number.isFinite(end)) throw new Error("Lịch mint không hợp lệ.");
...
export async function detectPresale(slug: string, contract: string, chain: string): Promise<boolean> {
  ...
  if (drop.chain !== chain || getAddress(drop.contract_address) !== getAddress(contract)) {
    throw new Error("Lịch OpenSea không khớp collection/chain đã chọn.");
  }
...
export async function runAllowlistWizard(checkOnly: boolean, existing?: { slug: string; key: string; quantity: number }): Promise<void> {
  const input = existing?.slug ?? await askText("Link OpenSea hoặc slug collection");
  const slug = input.startsWith("https://opensea.io/collection/")
    ? new URL(input).pathname.split("/")[2] : input;
  if (!/^[a-zA-Z0-9_-]+$/.test(slug)) throw new Error("Slug không hợp lệ.");
  const address = existing ? new Wallet(existing.key).address : getAddress(await askText("Địa chỉ ví công khai (0x...)") );
  const quantity = existing?.quantity ?? await askNumber("Số NFT", 1, { min: 1, max: 100 });
  ...
  try {
    const wallet = new Wallet(existing?.key ?? await askHidden("Private key của ví trên (nhập ẩn, chỉ giữ trong RAM): "), provider);
    if (wallet.address !== address) throw new Error("Private key không khớp ví đã kiểm tra.");
    const { tx } = checked;
    ...
    if (fees.maxFeePerGas === null || fees.maxPriorityFeePerGas === null) throw new Error("Không đọc được phí EIP-1559.");
    const maxCost = tx.value + gasLimit * fees.maxFeePerGas;
    if (await provider.getBalance(address) < maxCost) throw new Error("Ví thiếu số dư cho mint + gas tối đa.");
    console.log(`Ví: ${address}\nSeaDrop: ${tx.to}\nSố lượng: ${quantity}\nTiền mint: ${formatEther(tx.value)}\nTổng trần mint + gas: ${formatEther(maxCost)} ${checked.chain.nativeSymbol}`);
    if (!(await askYesNo(`Gửi giao dịch ${tx.method} ở stage ${tx.stage} này?`, false))) return;
    ...
    if ((await provider.getNetwork()).chainId !== BigInt(checked.chain.chainId)) throw new Error("RPC sai chain.");
    ...
    console.log(`Đã gửi: ${explorerTx(checked.chain.chainId, sent.hash)}`);
    const receipt = await sent.wait(1, 60_000);
    console.log(receipt?.status === 1 ? "Mint thành công." : "Chưa xác nhận thành công; kiểm tra explorer.");
  } finally {
    provider.destroy();
  }
}
