...
- **钱包视图（P1.2）**：执行器每 30 秒把「钱包地址 / 余额 / nonce」写入心跳文件（`queue/_heartbeat.json`，私钥不出执行器进程），serve 在「执行队列」页顶部渲染成一行：N 个钱包 · 各自余额与 nonce · **未来 45 分钟 N 个任务** · 同窗口冲突 M 对；执行器未运行时显示提示（余额/nonce 来自执行器，serve 不持有密钥也不直接读 RPC）
