...
npm start -- --refresh-targets --force-plan --limit 500 # 一次性重读所有已开售目标的公售价格/上限（清理历史旧值）
```
...
- **筛项目的三桶法与预设（review20）**：面板新增「桶」与「价格」两个下拉 + 「排除改价」（原来的秒空过滤扩为**秒空/批量**）：**A 桶**＝有真实需求但公售还有货（Q ≥ 50、剩余 ≥ 30%、风险标记干净），**B 桶**＝预计抢不到（instant-sellout / 预售 ≥ 60% / 剩余 ≤ 15% / cap 1 且地址 ≥ 1000），**C 桶**＝无人要（无预售 + 速度 0 + 地址 < 20）。两个预设按钮：**「A 桶：有货可达」**（A 桶 + Q≥50 + 未开售/新开售 + 排除秒空/批量/改价）与 **「低价 ≤0.001」**（价格 ≤ 0.001 ETH——低价付费项目机器人最少、余量最多，三次失败的复盘全是免费项目）。入队时把当时的**等级/Q 快照**一并写入任务，供两周后用回填数据校准
- **钱包视图（P1.2）**：执行器每 30 秒把「钱包地址 / **各链余额** / nonce / 预留」写入心跳文件（`queue/_heartbeat.json`，私钥不出执行器进程），serve 在「执行队列」页顶部渲染成一行：N 个钱包 · 各自余额与 nonce · **未来 45 分钟 N 个任务** · 同窗口冲突 M 对；执行器未运行时显示提示（余额/nonce 来自执行器，serve 不持有密钥也不直接读 RPC）
- **入队预览（P1.1）**：点「加入执行队列」时先调 `POST /api/queue/preview`，返回**预计数量**（与执行侧同一套数量策略，基于状态里的价格/上限）、**最坏花费**（gas 上限随数量放大后 × maxFee + 付费时的 value×N）、**风险标签**（instant-sellout / batch-mint / 尚未钉 codeHash / 尚无链上计划）与**同窗口冲突**（与队列中开售相差 ≤5s 的任务，因为所有任务共用钱包）；不适用目标（非 SeaDrop 公售）直接拒绝入队并给出原因。结果先显示在队列状态行，再真正入队
- **并行认领（P2）**：执行器一次认领 45 分钟窗口内的全部任务（`CLAIM_BATCH`，默认 8），按链合成**一个 parallel 批次**交给 runner——准备阶段重叠，发送阶段由钱包通道串行（同钱包只等发送、不再等整个任务结束）。批量心跳/续租覆盖组内全部任务；取消只作用于自己那个任务（`abortFor` 按 `chain|contract` 映射）；结果仍逐个从账本派生并通知
- **公开阶段价格保鲜（review20 修复）**：价格与开售时间**本来就只来自 `getPublicDrop`（公售阶段）**，白名单/GTD 阶段不参与——真正的问题是「已开售且没有新事件」的目标不在任何刷新路径里，而项目方常见的套路正是**开售后把免费改成收费**（你遇到的 Hashvein / Pupinhood / OVINE 都是）。现在：①`needsRefresh` 对「已开售、未售罄、事实超过 `PRICE_REFRESH_HOURS`（默认 6 小时）」的目标自动补读（每合约 2 次 RPC，不扫日志）；②扫描器每轮额外把 `REAUDIT_LIVE_PER_TICK`（默认 10）个最久未读的 live 目标送审，速度/售罄预计随之复活；③价格或上限真变了会写 `mintPriceChangedAt` 与 `priceHistory`（最多 5 条），面板价格列加「**改价**」徽标、展开明细显示变更轨迹；④**免费→收费**或**开售前后 1 小时内改价**记为 `price-flip` 惩罚，且「仅免费」过滤**排除**这些目标（免费榜诱饵不再被当成免费）；⑤面板按**数据新鲜度**取值（`factsAt` vs 审计时间），旧快照不再压住新审计
...
- **面板自动刷新（P0）**：状态栏每 15 秒轮询 `/api/status`，发现 `lastScanAt` 变化即 `location.reload()`（浏览器会暂停后台标签的 `<meta refresh>`，这是页面显示旧等级/旧阶段的根因）；保留 5 分钟 meta 兜底。- **价格与上限在发现阶段落盘（P0）**：候选过滤本来就要读一次 SeaDrop 计划，现在顺手写进状态（`mintPriceWei/capPerWallet/feeRecipient`，零额外 RPC）——「价格未知」积压随之消失，「仅免费」过滤不再误伤；`--refresh-targets` 同样补齐这几个字段。- **等级只在有链上计划时显示（P0）**：`mintPriceWei/capPerWallet` 均缺失的行显示 `?` 并按未评级参与筛选，避免「B 级却没有任何参数」的误导。
