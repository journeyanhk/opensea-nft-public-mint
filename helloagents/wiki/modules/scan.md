...
**模块:** gates / local-mint / batch-config / serve
- `src/gates.ts` 纯函数：`validateSignedTx`、`validateMintPublicCalldata`、`decodeSeaDropError`（按选择器解码，签名经 4byte 核对；含两种 NotActive 元数）、`revertDataOf`（`data`/`info.error.data`/message 三种形态）、`classifySimulation`（解码优先 → 文本兜底 → 开售前放行/开售后阻断）、`codeHashOf`
- `codeHash` 由 audit 读取并落状态、`--export` 写进 targets.json；`batch-config` 用 `targetCodeHash` 只接受真实哈希；`DEFAULT_REFRESH_MS = 5000`
- `local-mint` 顺序：Gate 1（codeHash）→ **Gate 3（仅开售后、并行；开售前跳过）** → 签名 → Gate 2（复核）→（dry-run 到此为止）→ 等待开售 → 齐发；`--dry-run` 不写账本且余额不足只警告
- `POST /api/favorites` 校验：链白名单（`resolveChain`）、合约 `^0x[0-9a-f]{40}# scan 模块
...
、note/slug 长度与 snapshot ≤4KB（收藏是分析数据集，拒绝脏数据）
- **已知盲区（未做）**：门 1 只哈希代理字节码，EIP-1967 透明/UUPS 代理换实现时会给假阴性；EIP-1167 最小代理因实现地址内嵌在字节码中仍有效。后续可加实现槽位/实现地址哈希
