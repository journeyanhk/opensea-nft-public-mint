...
- 时机选项：等待开售（T-0 发送）/立即发送/自定义 HH:MM（越南时间 UTC+7）
- 余额检查按 `gasLimit × maxFee + mint 金额` 预占口径

## API接口
### 导出
- `runWizard()`：公售路径主流程
- 依赖：prompt（askChoice/askHidden/askNumber/askText/askYesNo）、time-format（toVNTime/vnTimeToDate）、nft-link、slug-resolver、rpc-resolver、seadrop-public、local-mint、allowlist
