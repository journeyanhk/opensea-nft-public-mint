...
- 供应量检查（签名前）：读 NFT 合约 `getMintStats`（非 SeaDrop 单例，单例会 revert）；全局剩余 ≤ 0 → 该目标 SKIPPED（白名单常把公售库存提前清空）；剩余 < 请求总量 → 警告；单钱包 `已铸 + quantity > maxTotalMintableByWallet` → 该钱包从本次发送中剔除（保留原 idx，结果记 SKIPPED），全部被剔除则整目标 SKIPPED；合约不响应时返回 null，不阻塞。批量路径在 T-refresh 执行，向导路径在签名前执行一次
...
- [202609171345_arc-chain](../../history/2026-09/202609171345_arc-chain/) - checkSupply 抽为共用函数，向导路径同样拦截已售罄阶段
