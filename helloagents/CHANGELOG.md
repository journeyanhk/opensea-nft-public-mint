...
### 修复
- review20 三处：①钱包余额改为**按链**读取（`SCAN_CHAINS`，心跳 `wallets[].balances[{chain,wei,symbol}]` + `nonces`），Arc 的 USDC 余额不再隐形；②批次通道的**预留随心跳发布**（runner `onLaneSnapshot` → 执行器 `reservations`），面板显示「余额（预留 X）」；③面板钱包行**转义地址**、nonce 读取失败显示 `—` 而不是 -1
### 新增
- review20 筛选：面板「桶」下拉（A 有货可达 / B 预计抢不到 / C 无人要，规则纯前端由现有字段推导）+「价格」下拉（仅免费 / ≤0.001 / ≤0.01）+「排除改价」；两个预设「A 桶：有货可达」「低价 ≤0.001」；入队时写入等级与 Q 快照
...
### 修复
- 免费误标（review20）：根因不是白名单阶段混入（价格/开售时间只来自 `getPublicDrop`），而是**已开售且无新事件的目标没有任何刷新路径** —— 项目方常见的免费→收费改价发生在扫描窗口之外，面板一直显示发现时的旧快照。修复：①`needsRefresh` 增加"已开售未售罄 + factsAt 超过 `PRICE_REFRESH_HOURS`（6h）"自动补读；②扫描器每轮把最久的 `REAUDIT_LIVE_PER_TICK`（10）个 live 目标送审（速度/售罄预计复活）；③`price-watch.ts` 记录 `factsAt/mintPriceChangedAt/priceHistory`（≤5），面板价格列加「改价」徽标、明细显示轨迹；④免费→收费或开售前后 1 小时内改价 → `price-flip` 惩罚，且「仅免费」过滤排除；⑤面板价格/上限按**数据新鲜度**取值（不再让旧入口压住新审计）；⑥新增 `--refresh-targets --force-plan` 用于一次性清理存量旧值- 不适用审计被当成 B 级：`audit.ts` 的「no SeaDrop public drop」提前返回硬编码 `grade: "B"`，面板因此出现「B 级却无参数」、执行器日志先打 `grade B` 再报 `no codeHash could be pinned`（掩盖真实结论）。现在：①该分支返回无效等级 `?`（不再伪装成评级）；②`scanner` 仅对 `applicable && publicDrop` 的审计写入 `lastGrade/codeHash`，且不适用目标不写等级历史；③执行器新增 `applicabilityError`，**不适用即按真实原因失败**并通知；④`chainOnlySnapshot` 对空代码明确报 `no contract code at … on <chain> (wrong chain or address?)`；⑤CLI 批量预审与扫描汇总同样跳过不适用目标
- review19 两处：① `keepWarm` 提前到开售前 `max(leadMs,300)`ms 停止（原来 ping 可能恰好在第一发前几毫秒在飞，undici 会另开连接、反成冷握手），间隔 2s→1.5s；② 补齐 `.env.example`（FREE_MAX_QUANTITY/BURST_COUNT/BURST_AUTO_FREE_CAP1/BURST_ALLOW_OVERSHOOT/NOTIFY_*）与 `.env.serve.example`（NOTIFY_* 与为 P1.3 预留的 WALLET_ADDRESSES）
- arm token 改为固定：`loadOrCreateArmToken` 首次生成写入 `queue/arm-token`（0600）、重启沿用；`publishArmToken` 对同一 token 保留未过期的武装窗口（重启不再需要重新武装、也不会丢失武装状态）；`--rotate-arm-token` 手动轮换并解除武装；武装时长由 `EXECUTOR_ARM_TTL_H`（默认 12h）控制
- 数量策略抬高数量后 gas 上限仍是配置值（默认 250k，实测 1 枚约 124k gas）会导致必然 out-of-gas：新增 `gasLimitForQuantity`（每枚 150k + 60k 余量，只增不减，与余额预留方向一致），签名/门 2/补洞交易统一使用 `effectiveGasLimit`，并打印 `gas limit raised …`- 执行器快照锁定不再被无关故障拖垮：`auditTarget` 失败时回退到 **链上只读快照**（`chainOnlySnapshot`：`getCode` + SeaDrop 计划，不依赖 OpenSea），两者都失败时把底层原因合并写进任务 `error`，日志中的 RPC 用 `maskRpc` 脱敏；线上症状是公共 RPC 返回 HTML 导致 JSON 解析报错- review17（B5 上线前）：① **面板入队的 `maxPriceEth: "current"` 在执行器侧从不解析**，`parseEther("current")` 会让每个队列任务 100% 失败——现在认领时按审计快照解析成具体上限、写回任务文件（`resolveMaxPriceEth`），解析不到就拒绝执行；② 认领顺序改为**开售时间优先**（原按入队时间，早入队的远期任务会挡住临近任务），窗口 2h→45min、租约 15→30min；③ 执行期间每 30 秒**刷新心跳与续租**（原 `runBatch` 运行 30–120 分钟期间心跳停更、租约过期会被回收）；④ **取消标记在签名前生效**（`shouldAbort` 贯穿 runner → local-mint，拿到新 plan 后、签名前检查）；⑤ `--dry-run` 不再清除正在运行执行器的武装状态；⑥ 认领时先查账本，终态条目直接标 `skipped`；⑦ `.env.executor` 的 `BURST_COUNT/BURST_ALLOW_OVERSHOOT` 可为队列任务开启 burst；⑧ 入队限流 10/h- 队列目录里的非任务 JSON（`ARMED.json`/`executor-heartbeat.json` 等）被 `listJobs` 当成任务，排序时 `createdAt` 为 undefined 触发 `localeCompare` 崩溃（serve 页面与 `/api/queue` 报 500）：① 增加 `isJob` 形状判别，凡不具备 `id/createdAt/status/contract` 的文件一律忽略（`listJobs`/`claimNext`/`nextEligible`/`reclaimStale`/`cancelJob`/`updateJob` 全部改走判别）；② 基础设施文件改名 `_armed.json`/`_heartbeat.json`（旧名仍可读，向后兼容）；③ 排序空值兜底；新增回归用例（含损坏 JSON 与无关文件）
