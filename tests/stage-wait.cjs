...
test('stops when no future stage exists', async () => {
  await assert.rejects(waitForEligibleStage({check:async()=>{throw new MintApiError(422,'unavailable');},schedule:async()=>({stages:[]}),now:()=>0,sleep:async()=>{},log:()=>{}}),/Không có vòng/);
});
